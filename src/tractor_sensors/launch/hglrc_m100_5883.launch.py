...
def generate_launch_description():
    ...
    declare_gps_port_cmd = DeclareLaunchArgument(
        "gps_port", default_value="/dev/ttyS6", description="GPS serial port device"
    )

    declare_i2c_bus_cmd = DeclareLaunchArgument(
        "i2c_bus", default_value="5", description="I2C bus number for QMC5883 compass"
    )

    # Config file path
    config_file = os.path.join(
        pkg_tractor_sensors, "config", "hglrc_m100_5883_config.yaml"
    )
    ...
    hglrc_node = Node(
        ...
        parameters=[
            config_file,
            {
                ...
                "gps_port": gps_port,
                "i2c_bus": (
                    int(i2c_bus.perform_substitution(context=None))
                    if hasattr(i2c_bus, "perform_substitution")
                    else 5
                ),
            },
        ],
