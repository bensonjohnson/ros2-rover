...
try:
    ...
    print(
        "pyrealsense2 not available. Will use official realsense2_camera node instead."
    )
...
class RealSenseProcessor(Node):
    def __init__(self):
        ...
        if not REALSENSE_AVAILABLE:
            self.get_logger().info(
                "pyrealsense2 library not available. Using ROS2 RealSense topics instead."
            )
        ...
        if self.enable_pointcloud:
            ...
            self.decimation.set_option(
                rs.option.filter_magnitude, self.pointcloud_decimation
            )
    ...
    def setup_ros_subscribers(self):
        ...
        self.color_sub = self.create_subscription(
            Image, f"/{self.camera_name}/color/image_raw", self.color_callback, 10
        )
        ...
        self.processed_color_pub = self.create_publisher(
            Image, f"{self.camera_name}/processed/color/image_raw", 10
        )

        self.processed_depth_pub = self.create_publisher(
            Image, f"{self.camera_name}/processed/depth/image_raw", 10
        )
    ...
    def pointcloud_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().debug(
                "Received point cloud with {} points".format(msg.width * msg.height)
            )
    ...
    def process_frames(self):
        ...
        try:
            ...
            depth_msg = self.bridge.cv2_to_imgmsg(
                depth_image_filtered, encoding="16UC1"
            )
    ...
    def publish_camera_info(self, color_frame, depth_frame, timestamp):
        ...
        depth_info.d = [
            depth_intrinsics.coeffs[0],
            depth_intrinsics.coeffs[1],
            depth_intrinsics.coeffs[2],
            depth_intrinsics.coeffs[3],
