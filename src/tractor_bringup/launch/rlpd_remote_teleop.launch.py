"""RLPD + HIL-SERL teleop collection launch.

Bootstraps the demo replay buffer for the RLPD trainer by collecting
teleop-driven chunks (RKNN inference disabled). All chunks shipped to the
server are marked `is_demo=True`. After enough collection, switch to
`rlpd_remote_autonomous.launch.py` for autonomous training with HIL-SERL
interventions.

Mirrors `dreamer_remote_teleop.launch.py` — only the runner executable and
the launch defaults differ.
"""
