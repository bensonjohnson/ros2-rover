...
def generate_launch_description():
    ...
    max_episode_steps_arg = DeclareLaunchArgument(
        'max_episode_steps',
        default_value='1000',
        description='Maximum steps per episode (~33s @ 30Hz)'
    )
    ...
    # 7. ES-SAC Episode Runner Node
    es_sac_runner_node = Node(
        package='tractor_bringup',
        executable='es_episode_runner.py',
        name='es_episode_runner',
        output='screen',
        parameters=[{
    ...
    return LaunchDescription([
        ...
        # AI (last)
        TimerAction(period=10.0, actions=[es_sac_runner_node])
    ])
