...
class FrontierExplorer:
    def __init__(self, res: float = 0.15, size_m: float = 24.0,
                 ...
                 stuck_window: int = 30, stuck_dist: float = 0.05):
        ...
        self.escape = 0
        self.edir = 1.0
        # Stuck detection: physical progress over a rolling window.
        self.stuck_window = stuck_window
        self.stuck_dist = stuck_dist
        self._recent: deque = deque(maxlen=stuck_window)
    ...
    def step(self, x, y, theta, ranges, bearings, blocked: bool = False):
        ...
        info = {"target_bearing": rad robot-relative (or None),
                "exploring": bool, "stuck": bool}. `blocked` should be the
        safety gate's front-clamp state — the authority on whether forward is
        allowed; when set we commit to an escape pivot for several ticks
        (re-deciding every tick just re-wedges against the wall)."""
        self.update(x, y, theta, ranges, bearings)
        self.k += 1

        # STUCK DETECTION. `blocked` is not sufficient: the safety gate clears
        # a corridor only robot_half_width (0.12 m) wide, which is NARROWER
        # than the robot's collision radius (0.14 m), so a corner just outside
        # the corridor can hold the rover fast while the gate reports clear.
        # Measured in sim: one house in twelve wedged like this and drove
        # forward into it for the entire 5-minute run — 1.8 m travelled, 0%
        # blocked, a valid path the whole time. Physical progress is the only
        # honest test, so escape on lack of motion too.
        # Only lack of motion while COMMANDING FORWARD DRIVE counts. Pivoting
        # in place and escape pivots are supposed not to translate — an
        # earlier version ignored that, fired constantly (pivot-in-place is
        # ~40% of ticks), and cut mean travel from 8.7 m to 7.9 m by
        # interrupting good plans.
        stuck = False
        if self._was_driving:
            self._recent.append((float(x), float(y)))
            if len(self._recent) >= self.stuck_window:
                x0, y0 = self._recent[0]
                stuck = np.hypot(x - x0, y - y0) < self.stuck_dist
            if stuck:
                self._recent.clear()
        else:
            self._recent.clear()
        ...
        if self.escape > 0 or blocked or stuck:
            if self.escape <= 0:
        ...
        info = {"target_bearing": float(tb), "exploring": True, "stuck": False}
