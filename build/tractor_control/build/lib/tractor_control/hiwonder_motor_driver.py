...
class HiwonderMotorDriver(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_motor_speed', 50)  # Reduced from ESP32 testing
        ...
        self.declare_parameter('use_pwm_control', True)  # Use PWM instead of speed control
        self.declare_parameter('motor_type', 0)  # 0=no encoder, 1=TT, 2=N20, 3=JGB37
    ...
    def init_motor_driver(self):
        ...
        try:
            # Set motor type based on parameter
            self.write_data_array(self.MOTOR_TYPE_ADDR, [self.motor_type])
            ...
            # Set encoder polarity (official sample uses 0, but may need 1 for some motors)
            encoder_polarity = 0  # Try 1 if motors don't respond
            self.write_data_array(self.MOTOR_ENCODER_POLARITY_ADDR, [encoder_polarity])
    ...
    def write_data_array(self, reg, data):
        """Write data array to I2C register - using sequential single-byte protocol"""
        try:
            # This motor controller requires sequential single-byte writes:
            # 1. Write register address first
            # 2. Write each data byte individually
            
            if len(data) == 1:
                # Single byte write using standard method
                self.bus.write_byte_data(self.motor_address, reg, data[0])
            else:
                # Multi-byte write using sequential method that works:
                # Step 1: Write register address
                self.bus.write_byte(self.motor_address, reg)
                time.sleep(0.001)  # Small delay
                
                # Step 2: Write each data byte individually
                for byte_val in data:
                    self.bus.write_byte(self.motor_address, byte_val)
    ...
    def send_motor_speeds(self, left_speed, right_speed):
        ...
        try:
            # Pack speeds as signed 8-bit integers (M1=left, M2=right, M3=0, M4=0)
            speeds = [left_speed, right_speed, 0, 0]
            # Convert to signed 8-bit values
            speeds_bytes = [int(s) if -128 <= s <= 127 else max(-128, min(127, int(s))) for s in speeds]
            ...
            # Send to motor controller at correct address
            if self.write_data_array(control_addr, speeds_bytes):
                if left_speed != 0 or right_speed != 0:
                    self.get_logger().info("✅ Motor command sent successfully")
                self.last_motor_command_time = current_time
            else:
                self.get_logger().error("❌ Failed to send motor speeds")
