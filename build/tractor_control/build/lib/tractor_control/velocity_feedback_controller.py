...
class VelocityFeedbackController(Node):
    def __init__(self):
        ...
        self.declare_parameter('kp_angular', 0.6)  # Proportional gain for angular velocity
        self.declare_parameter('ki_linear', 0.1)  # Integral gain for linear velocity
        self.declare_parameter('ki_angular', 0.05)  # Integral gain for angular velocity
        ...
        self.cmd_vel_pub = self.create_publisher(
            Twist,
            'cmd_vel',  # Output: corrected velocity command
            10
        )
        ...
        self.get_logger().info(f'Velocity Feedback Controller initialized at {self.control_frequency} Hz')
        self.get_logger().info(f'PID gains - Linear: Kp={self.kp_linear}, Ki={self.ki_linear}')
        self.get_logger().info(f'PID gains - Angular: Kp={self.kp_angular}, Ki={self.ki_angular}')
    ...
    def emergency_stop_callback(self, msg):
        ...
        if self.emergency_stop:
            with self.control_lock:
                ...
                self.linear_integral = 0.0
                self.angular_integral = 0.0
            self.get_logger().warn('Emergency stop activated - clearing velocity commands')
    ...
    def control_loop(self):
        ...
        with self.control_lock:
            ...
            if abs(self.desired_linear) > 0.01:  # Only integrate when moving
                self.linear_integral += linear_error * dt
                self.linear_integral = max(-self.max_integral, 
                                         min(self.max_integral, self.linear_integral))
            else:
            ...
            if abs(self.desired_angular) > 0.01:  # Only integrate when turning
                ...
                self.angular_integral = max(-self.max_integral, 
            ...
            if abs(linear_error) > 0.05 or abs(angular_error) > 0.05:
                self.get_logger().debug(
                    f'PID: Des[{self.desired_linear:.2f}, {self.desired_angular:.2f}] '
                    f'Act[{self.current_linear:.2f}, {self.current_angular:.2f}] '
                    f'Err[{linear_error:.2f}, {angular_error:.2f}] '
                    f'Out[{corrected_linear:.2f}, {corrected_angular:.2f}]'
                )
