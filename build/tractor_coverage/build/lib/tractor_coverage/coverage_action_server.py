...
class CoverageActionServer(Node):
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info(
            f"Accepting coverage goal: {
                goal_request.operation_type}"
        )
    ...
    def execute_callback(self, goal_handle):
        ...
        try:
            ...
            result.total_time = Duration(
                sec=int(elapsed_time), nanosec=int((elapsed_time % 1) * 1e9)
            )
    ...
    def setup_operation(self, request):
        ...
        tool_width = (
            request.tool_width if request.tool_width > 0 else self.default_tool_width
        )
        overlap = (
            request.overlap_percentage
            if request.overlap_percentage >= 0
    ...
    def execute_waypoint_following(self, goal_handle, feedback_msg) -> bool:
        ...
        while self.current_waypoint_index < total_waypoints and self.is_executing:
            ...
            if not self.wait_for_waypoint_completion(
                current_pose, timeout=self.max_nav_timeout
            ):
    ...
    def wait_for_waypoint_completion(
        self, target_pose: PoseStamped, timeout: float
    ) -> bool:
    ...
    def publish_feedback(self, goal_handle, feedback_msg, total_waypoints: int):
        ...
        feedback_msg.percent_complete = (
            self.current_waypoint_index / total_waypoints
        ) * 100.0
        ...
        feedback_msg.current_status = f"Waypoint {
            self.current_waypoint_index}/{total_waypoints}"
