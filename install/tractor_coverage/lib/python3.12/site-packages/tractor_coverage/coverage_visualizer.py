...
class CoverageVisualizer(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.boundary_markers_pub = self.create_publisher(MarkerArray, 'boundary_markers', 10)
        self.path_markers_pub = self.create_publisher(MarkerArray, 'path_visualization', 10)
            ...
            if len(path_x) > 0:
                ...
                for i in range(0, len(path_x) - 1, max(1, len(path_x) // 20)):
                    ...
                    if abs(dx) > 0.01 or abs(dy) > 0.01:  # Avoid zero-length arrows
                        self.ax.arrow(path_x[i], path_y[i], dx * 0.3, dy * 0.3,
    ...
    def calculate_statistics(self) -> str:
        ...
        if self.current_path:
            ...
            if self.current_boundary:
                ...
                boundary_perimeter = self.calculate_polygon_perimeter(self.current_boundary.points)
