...
class SprayerController(Node):
    def __init__(self):
        ...
        self.tank_level_i2c_addr = self.get_parameter(
            "tank_level_sensor_i2c_addr"
        ).value
        self.pressure_sensor_i2c_addr = self.get_parameter(
        ...
        self.nozzle_status_pub = self.create_publisher(
            String, "sprayer/nozzle_status", 10
        )
    ...
    def nozzle_control_callback(self, msg):
        ...
        try:
            ...
            for i, state in enumerate(nozzle_states[: len(self.nozzle_pins)]):
                ...
                GPIO.output(
                    self.nozzle_pins[i],
                    GPIO.HIGH if self.active_nozzles[i] else GPIO.LOW,
                )
        except Exception as e:
    ...
    def cmd_vel_callback(self, msg):
        ...
        if self.sprayer_enabled and linear_speed > 0:
            # Calculate application rate (example: maintain constant volume per
            # area)
    ...
    def read_sensors(self):
        ...
        try:
            ...
            pressure_raw = self.i2c_bus.read_word_data(
                self.pressure_sensor_i2c_addr, 0x00
            )
    ...
    def publish_status(self):
        ...
        nozzle_msg = String()
        nozzle_msg.data = ",".join(
            ["1" if active else "0" for active in self.active_nozzles]
        )
