#!/usr/bin/env python3
"""
ROS 2 Node for Xbox controller based teleoperation.

This node reads input from a connected Xbox (or compatible) controller using pygame
and publishes geometry_msgs/Twist commands. It supports both tank drive and
arcade drive modes. It can also integrate with a velocity feedback controller
by publishing to a 'cmd_vel_raw' topic if configured. An emergency stop
feature is included.
"""
import rclpy
...
class XboxControllerTeleop(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"Xbox Controller Teleop initialized (Tank Drive: {
                self.tank_drive_mode}, Feedback Control: {
                self.use_feedback_control})"
        )
        self.print_controls()
    ...
    def print_controls(self):
        ...
        if self.use_feedback_control:
            self.get_logger().info(
                "- Encoder feedback control: ENABLED (auto-correction)"
            )
        else:
    ...
    def tank_drive_control(self, left_y, right_y):
        ...
        # For tank drive: angular_vel = (right_wheel_speed - left_wheel_speed)
        # / wheel_separation
        if self.wheel_separation > 0.01:  # Avoid division by zero or tiny numbers
            angular_vel = (right_wheel_speed - left_wheel_speed) / self.wheel_separation
        else:
            angular_vel = (
                0.0  # Default to no angular velocity if wheel_separation is invalid
            )
