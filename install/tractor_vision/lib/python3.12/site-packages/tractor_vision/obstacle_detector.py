...
class ObstacleDetector(Node):
    def __init__(self):
        ...
        self.obstacle_mask_pub = self.create_publisher(
            Image,
            'obstacle_mask',
            10
        )
    ...
    def filter_detection_area(self, points):
        ...
        height_mask = (points[:, 2] >= -0.5) & (points[:, 2] <= self.max_obstacle_height)
    ...
    def cluster_points(self, points):
        ...
        while len(remaining_points) > self.min_cluster_size:
            ...
            if len(cluster_points) >= self.min_cluster_size and len(cluster_points) <= self.max_cluster_size:
    ...
    def create_obstacle_mask(self, depth_image):
        ...
        obstacle_mask_roi = cv2.morphologyEx(
            obstacle_mask_roi.astype(np.uint8) * 255,
            cv2.MORPH_CLOSE,
            kernel
        )
