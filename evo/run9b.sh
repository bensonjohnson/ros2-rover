#!/usr/bin/env bash
# Run 9b: FIXED ES + FIXED POLICY READOUT — 6a replica (h128, w_cov 0.3,
...
OUT=evo_runs/greenfield9b
rm -rf $OUT; mkdir -p $OUT
echo "=== 9b (fixed ES + readout, 6a replica h128 w_cov 0.3) start $(date) ===" | tee -a $OUT/run.log
...
python3 -u -m evo.baselines --games 32 --device cuda --fp16 \
    ...
    >> $OUT/run.log 2>&1
echo "=== 9b rc=$RC done $(date) ===" | tee -a $OUT/run.log
echo "RUN9B-COMPLETE rc=$RC" | tee -a $OUT/run.log
